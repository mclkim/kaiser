...
class ErrorHandler {
	...
	/**
	 * All exceptions are piped through this method from the framework workflow.
	 ...
	 *
	 * @param Exception $proposedException
...
elseif (static::$activeMask !== null) {
	...
	public static function getDetailedMessage($exception) {
		...
		if (! ($exception instanceof ApplicationException) && Config::get ( 'app.debug', false )) {
			return sprintf ( '"%s" on line %s of %s', $exception->getMessage (), $exception->getLine (), $exception->getFile () );
		} else {
