...
class ExceptionBase extends Exception
{
    ...
    /**
     * CMS base exception class constructor.
     * Inherits the native PHP Exception.
    ...
    /**
     * Masks an exception with the called class.
     * This should catch fatal and php errors.
    ...
    /**
     * If this exception is acting as a mask, return the face exception.
     * Otherwise return
    ...
    {
        $result = [];
        $traceInfo = $this->filterCallStack($this->getTrueException()
            ->getTrace());
    ...
    protected function filterCallStack($traceInfo)
    {
        ...
        foreach ($traceInfo as $event) {
            if (isset($event['class']) && $event['class'] == 'Illuminate\Foundation\Bootstrap\HandleExceptions' && $event['function'] == 'handleError') {
                $useFilter = true;
            }
        }
